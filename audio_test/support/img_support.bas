...
'declare sub img_transfer(sourceImg as any ptr, targetImg as any ptr, x as integer, y as integer, w as integer, h as integer)
declare sub img_transfer(targetImg as any ptr, y as integer, w as integer, bpp as integer)

sub img_transfer(targetImg as any ptr, y as integer, w as integer, bpp as integer)
    'dim as integer rowBytePos, colBytePos
    'dim as uinteger ptr sourceRow, targetRow
    
    'sourceRow = screenPtr + y * w * bpp
    'targetRow = targetImg + y * w * bpp
    ...
    get targetImg, (0,y)-(w,y), screenPtr
end sub
...
Sub img_Scroll(img As Any Ptr, x1 As Integer, y1 As Integer, dX As Integer, dY As Integer, msStartInterval As Integer = defaultStartInterval, msEndInterval As Integer = defaultEndInterval)
    dim as integer maxDelta, x, y, dXIsMax, dXSign, dYSign, dXProgress, dYProgress, fX, fY
    ...
    for i as integer = 0 to maxDelta
        if (Inkey = imgSkipKey) then 
            put(x1 + (dX * dXSign), y1 + (dY * dYSign)), img, pset
            return
        end if
